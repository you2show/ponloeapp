...
import { fiqhData } from './data';

interface FiqhViewProps {
  setView: (view: ViewMode) => void;
}

export const FiqhView: React.FC<FiqhViewProps> = ({ setView }) => {
    ...
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-slate-950 text-white' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <header className={`sticky top-0 z-30 px-4 py-4 flex items-center gap-4 border-b backdrop-blur-md ${
        ...
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold font-khmer-title">ហ្វីកោះ (ក្បួនច្បាប់អ៊ីស្លាម)</h1>
        </div>
      </header>
