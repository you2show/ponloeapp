...
import { aqeedaData } from './data';

interface AqeedaViewProps {
  setView: (view: ViewMode) => void;
}

export const AqeedaView: React.FC<AqeedaViewProps> = ({ setView }) => {
    ...
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-slate-950 text-white' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <header className={`sticky top-0 z-30 px-4 py-4 flex items-center gap-4 border-b backdrop-blur-md ${
        ...
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold font-khmer-title">អាគីហ្ទះ (គោលជំនឿ)</h1>
        </div>
      </header>
