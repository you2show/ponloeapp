...
import { moralityData } from './data';
import { ViewMode } from '../../types';

interface MoralityViewProps {
  setView: (view: ViewMode) => void;
}

export const MoralityView: React.FC<MoralityViewProps> = ({ setView }) => {
    ...
    <div className={`min-h-screen pb-24 ${theme === 'dark' ? 'bg-slate-950' : 'bg-gray-50'}`}>
      ...
      <div className="max-w-3xl mx-auto p-4 space-y-4">
        ...
        <div className={`p-6 rounded-2xl border ${
          theme === 'dark' 
            ? 'bg-cyan-900/20 border-cyan-800/50 text-cyan-100' 
            : 'bg-cyan-50 border-cyan-100 text-cyan-900'
        }`}>
          <div className="flex items-center gap-4 mb-3">
            <div className={`p-3 rounded-xl ${
              theme === 'dark' ? 'bg-cyan-800/50 text-cyan-300' : 'bg-cyan-100 text-cyan-600'
            }`}>
              <BookOpen size={24} />
            </div>
            <h2 className="text-lg font-bold font-khmer">សេចក្តីផ្តើម</h2>
          </div>
          <p className={`font-khmer leading-relaxed ${theme === 'dark' ? 'text-cyan-200' : 'text-cyan-800'}`}>
            ផ្នែកនេះនឹងបង្ហាញអំពីសីលធម៌ និងគុណធម៌ដ៏ល្អប្រពៃដែលអ្នកមូស្លីមគ្រប់រូបគប្បីប្រតិបត្តិនៅក្នុងជីវិតប្រចាំថ្ងៃ។
          </p>
        </div>

        {/* Questions List */}
        <div className="space-y-3">
          {moralityData.map((item, index) => (
            <motion.div
              key={index}
