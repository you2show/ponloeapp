...
import { basicHadithData } from './basic-hadith-data';

interface BasicHadithViewProps {
  setView: (view: ViewMode) => void;
}

export const BasicHadithView: React.FC<BasicHadithViewProps> = ({ setView }) => {
    ...
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-slate-950 text-white' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <header className={`sticky top-0 z-30 px-4 py-4 flex items-center gap-4 border-b backdrop-blur-md ${
        ...
        <div className="flex items-center gap-3">
          <div className={`p-2 rounded-xl ${theme === 'dark' ? 'bg-rose-500/20 text-rose-400' : 'bg-rose-100 text-rose-600'}`}>
            <BookOpen className="w-5 h-5" />
          </div>
          <h1 className="text-xl font-bold font-khmer-title">ផ្នែកហាទីស្ហ (ប្រសាសន៍របស់ព្យាការី)</h1>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto p-4 md:p-6 pb-24">
        <div className="space-y-4">
          {basicHadithData.map((item, index) => {
            ...
            return (
              <div 
                ...
                } ${isOpen ? 'ring-2 ring-rose-500/50' : ''}`}
              >
                <button
