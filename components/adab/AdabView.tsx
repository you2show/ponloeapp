...
import { adabData } from './data';
import { ViewMode } from '../../types';

interface AdabViewProps {
  setView: (view: ViewMode) => void;
}

export const AdabView: React.FC<AdabViewProps> = ({ setView }) => {
      ...
      <div className="max-w-3xl mx-auto p-4 space-y-4">
        ...
        <div className={`p-6 rounded-2xl border ${
          ...
          <p className={`font-khmer leading-relaxed ${theme === 'dark' ? 'text-cyan-200' : 'text-cyan-800'}`}>
            ផ្នែកនេះនឹងបង្ហាញអំពីសុជីវធម៌ និងសីលធម៌ដ៏ល្អប្រពៃដែលអ្នកមូស្លីមគ្រប់រូបគប្បីប្រតិបត្តិនៅក្នុងជីវិតប្រចាំថ្ងៃ។
          </p>
        </div>

        {/* Questions List */}
        <div className="space-y-3">
          {adabData.map((item, index) => (
            <motion.div
              key={index}
