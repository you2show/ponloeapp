    ...
    <div className="h-full px-4 py-3 flex items-center justify-between bg-white dark:bg-slate-900">
      {isSearchExpanded ? (
        <div className="flex items-center w-full gap-2 animate-in fade-in slide-in-from-right-4 duration-200">
           <button onClick={() => setIsSearchExpanded(false)} className="p-2 -ml-2 text-gray-500 dark:text-slate-400">
  ...
  return (
    <div className="h-full border-b shadow-sm transition-all duration-300 bg-white border-gray-200 dark:bg-slate-900 dark:border-slate-800">
       <div className="h-full flex items-center justify-start sm:justify-center gap-2 px-3 py-2 overflow-x-auto no-scrollbar snap-x">
          ...
          >
            {profile?.avatar_url ? (
              <img src={profile.avatar_url} alt="Profile" className="w-6 h-6 rounded-full object-cover" referrerPolicy="no-referrer" loading="lazy" decoding="async" />
            ) : (
      ...
      if (data && data.length > 0) {
        ...
        const mappedPosts = data.map((post: any) => {
          ...
          return {
            ...
            audioData: extraData?.tracks ? extraData : (extraData?.audioData || post.audioData),
            ...extraData
          };
        });
        ...
        <div className={`w-full px-0 md:px-2 pt-4 ${activeTab === 'chat' ? 'pb-0' : 'pb-24 md:pb-8'} ${activeTab === 'profile' || activeTab === 'chat' || activeTab === 'watch' || isReadingBook ? 'max-w-4xl' : 'max-w-[600px]'}`}>
           ...
           {activeTab === 'ustaz' && (
             <div className="animate-in fade-in duration-300 px-2 md:px-0">
                ...
                {loadingUstaz ? (
                  <div className="text-center py-8 text-gray-500 dark:text-slate-400">កំពុងផ្ទុក...</div>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    {ustazList.length === 0 && !loadingUstaz && (
                      <div className="col-span-full text-center py-8 text-gray-500 dark:text-slate-400">មិនទាន់មានទិន្នន័យនៅឡើយទេ</div>
                    )}
                  </div>
                )}
             </div>
           ...
           {activeTab === 'market' && (
             <div className="animate-in fade-in duration-300 px-2 md:px-0">
                ...
                <div className="grid grid-cols-2 gap-3">
                   {loadingMarket ? (
                     <div className="col-span-2 text-center py-8 text-gray-500 dark:text-slate-400">កំពុងផ្ទុក...</div>
                     ...
                     marketItems.map((item, idx) => (
                      <div key={idx} className="rounded-xl overflow-hidden border shadow-sm hover:shadow-md transition-all cursor-pointer group bg-white border-gray-200 dark:bg-slate-900 dark:border-slate-800">
                         <div className="aspect-square relative overflow-hidden bg-gray-100">
                            ...
                            <div className="absolute top-2 right-2 bg-white/90 backdrop-blur px-2 py-1 rounded-lg text-xs font-bold shadow-sm text-emerald-700 dark:bg-slate-900">{item.price}</div>
                            <div className="absolute top-2 left-2">
                              <button className="p-1.5 bg-white/80 backdrop-blur-sm rounded-full hover:bg-white transition-colors dark:bg-slate-900">
                                <HugeiconsIcon icon={FavouriteIcon} strokeWidth={1.5} className="w-4 h-4 text-gray-500 dark:text-slate-400" />
                              </button>
                            </div>
                         </div>
                         <div className="p-3">
                            <h4 className="font-bold text-sm line-clamp-1 mb-1 font-khmer text-gray-900 dark:text-white">{item.title}</h4>
                            <div className="flex items-center gap-1 text-xs text-gray-500 mb-2 dark:text-slate-400">
                     ...
                     <div className="col-span-2 text-center py-8 text-gray-500 dark:text-slate-400">មិនទាន់មានទំនិញនៅឡើយទេ</div>
                   )}
                </div>
             </div>
           )}
