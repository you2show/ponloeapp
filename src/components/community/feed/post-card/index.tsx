...
export const PostCard: React.FC<{ 
  ...
  return (
    <div className={`rounded-xl shadow-sm border mb-4 overflow-hidden ${theme === 'dark' ? 'bg-slate-900 border-slate-800' : 'bg-white border-gray-200'} dark:bg-slate-900 dark:border-slate-700`}>
      ...
      <div className={`px-4 pb-3 ${hasBackground ? 'min-h-[200px] flex flex-col justify-center items-center text-center p-6' : ''}`} style={backgroundStyle}>
        ...
        {isLongText && (
            <button 
                ...
                className={`${hasBackground ? 'text-white/80 hover:text-white' : 'text-gray-500 hover:text-emerald-600'} text-xs font-bold mt-1 dark:text-slate-400`}
            >
                 ...
                 return (
                    ...
                    >
                       ...
                       <div className="absolute inset-0 flex items-center justify-between px-4 text-sm">
                          <span className={`font-medium z-10 font-khmer ${theme === 'dark' ? 'text-slate-200' : 'text-gray-800'} ${isSelected ? 'font-bold' : ''} dark:text-slate-200`}>
                            {opt.text} {isSelected && <span className="text-emerald-500 ml-2">✓</span>}
                          </span>
                          <span className={`font-bold z-10 ${theme === 'dark' ? 'text-slate-400' : 'text-gray-600'} dark:text-slate-400`}>{percent}%</span>
                       </div>
                    </div>
                 )
              })}
              <p className={`text-xs mt-1 text-right ${hasBackground ? 'text-white/80' : 'text-gray-500'} dark:text-slate-400`}>
        ...
        {post.type === 'book' && (post.bookData || post.extra_data?.bookData) && (
            <div className={`mt-3 p-4 rounded-2xl border flex gap-4 transition-all duration-300 ${theme === 'dark' ? 'bg-slate-800/50 border-slate-700 hover:bg-slate-800' : 'bg-white border-gray-100 shadow-sm hover:shadow-md'} dark:bg-slate-900 dark:border-slate-800`}>
                {/* Book Cover */}
                <div className={`w-24 h-36 shrink-0 rounded-xl overflow-hidden shadow-sm border relative group ${theme === 'dark' ? 'bg-slate-800 border-slate-700' : 'bg-gray-50 border-gray-100/50'} dark:bg-slate-800 dark:border-slate-800`}>
                ...
                <div className="flex-1 flex flex-col justify-center">
                    <div className="flex justify-between items-start gap-2">
                        <h3 className={`font-bold text-lg mb-1.5 line-clamp-2 leading-snug ${theme === 'dark' ? 'text-slate-200' : 'text-gray-900'} dark:text-white`}>
                    ...
                    <p className={`text-sm mb-3 font-medium ${theme === 'dark' ? 'text-slate-400' : 'text-gray-500'} dark:text-slate-400`}>
                    ...
                    <div className={`flex flex-wrap gap-3 text-xs mb-4 ${theme === 'dark' ? 'text-slate-500' : 'text-gray-400'}`}>
                        {(post.bookData?.pages || post.extra_data?.bookData?.pages) && (
                            <span className="flex items-center gap-1.5 bg-gray-50 px-2 py-1 rounded-md border border-gray-100/50 dark:bg-slate-800 dark:border-slate-800">
        ...
        {post.type === 'dua' && (post.duaData || post.extra_data?.duaData) && (
           <div className={`mt-3 p-6 rounded-xl border text-center relative overflow-hidden ${theme === 'dark' ? 'bg-amber-900/20 border-amber-800' : 'bg-amber-50 border-amber-100'}`}>
              ...
              <p className={`font-khmer text-base md:text-lg italic leading-relaxed ${theme === 'dark' ? 'text-slate-300' : 'text-gray-700'} dark:text-slate-300`}>
