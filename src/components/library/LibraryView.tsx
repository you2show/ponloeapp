...
import { libraryService, DownloadedBook } from '@/services/libraryService';
...
export const LibraryView: React.FC = () => {
  ...
  return (
    <div className="h-full bg-[#fafafa] flex overflow-hidden animate-in fade-in duration-300 font-khmer">
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-white/50 backdrop-blur-sm border-r border-gray-100 h-full shrink-0 pt-6 overflow-y-auto dark:bg-slate-900 dark:border-slate-800">
         <div className="px-6 mb-6">
            <h2 className="text-xl font-bold font-khmer-title text-gray-900 flex items-center gap-3 dark:text-white">
         ...
         <div className="px-4 space-y-2">
            ...
            >
               <HugeiconsIcon icon={Book02Icon} strokeWidth={1.5} className={`w-5 h-5 transition-colors ${filterMode === 'browse' ? 'text-emerald-500' : 'text-gray-400 group-hover:text-gray-600'} dark:text-slate-400`} /> 
               បញ្ជីសៀវភៅ
            </button>
            ...
            >
               <HugeiconsIcon icon={AudioBook04Icon} strokeWidth={1.5} className={`w-5 h-5 transition-colors ${filterMode === 'audiobook' ? 'text-emerald-500' : 'text-gray-400 group-hover:text-gray-600'} dark:text-slate-400`} /> 
               សៀវភៅសំឡេង
            </button>
            ...
            >
               <BookImageIcon className={`w-5 h-5 transition-colors ${filterMode === 'picturebook' ? 'text-emerald-500' : 'text-gray-400 group-hover:text-gray-600'} dark:text-slate-400`} /> 
               សៀវភៅរូបភាព
            </button>
            ...
            >
               <BookUserIcon className={`w-5 h-5 transition-colors ${filterMode === 'authors' ? 'text-emerald-500' : 'text-gray-400 group-hover:text-gray-600'} dark:text-slate-400`} /> 
               អ្នកនិពន្ធ
            </button>
            ...
            >
               <HugeiconsIcon icon={BookDownloadIcon} strokeWidth={1.5} className={`w-5 h-5 transition-colors ${filterMode === 'downloaded' ? 'text-emerald-500' : 'text-gray-400 group-hover:text-gray-600'} dark:text-slate-400`} /> 
               បានទាញយក
               {downloadedIds.size > 0 && (
                  <span className="ml-auto bg-gray-100 text-gray-600 text-[10px] px-2 py-0.5 rounded-full font-mono dark:text-slate-400">{downloadedIds.size}</span>
               )}
            </button>
            ...
            >
               <HugeiconsIcon icon={BookBookmark02Icon} strokeWidth={1.5} className={`w-5 h-5 transition-colors ${filterMode === 'saved' ? 'text-emerald-500' : 'text-gray-400 group-hover:text-gray-600'} dark:text-slate-400`} /> 
               បានរក្សាទុក
               {savedIds.size > 0 && (
                  <span className="ml-auto bg-gray-100 text-gray-600 text-[10px] px-2 py-0.5 rounded-full font-mono dark:text-slate-400">{savedIds.size}</span>
               )}
            </button>
         </div>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative">
         ...
         <div className="px-6 py-2 shrink-0 z-20">
            <div className="flex items-center gap-2 bg-white py-1.5 px-2 rounded-full shadow-sm border border-gray-100 max-w-3xl relative dark:bg-slate-900 dark:border-slate-800">
               <button 
                  onClick={() => setIsMobileMenuOpen(true)}
                  className="lg:hidden p-2 bg-gray-50 text-gray-600 hover:bg-gray-100 rounded-full transition-colors border border-gray-100 shadow-sm mr-1 dark:bg-slate-800 dark:border-slate-800 dark:text-slate-400"
               >
               ...
               <input 
                  ...
                  className="flex-1 bg-transparent border-none focus:ring-0 text-sm font-medium text-gray-800 placeholder-gray-400 outline-none h-full py-1.5 min-w-0 dark:text-slate-200"
               />
               ...
               <div className="relative" ref={filterRef}>
                   <button 
                      ...
                      className="hidden md:flex items-center gap-2 px-4 py-2 text-gray-600 hover:bg-gray-50 rounded-2xl transition-colors text-sm font-bold dark:bg-slate-800 dark:text-slate-400"
                   >
                   ...
                   <button 
                      onClick={() => setIsFilterMenuOpen(!isFilterMenuOpen)}
                      className="md:hidden p-2 bg-gray-100 rounded-full text-gray-600 hover:bg-gray-200 dark:text-slate-400"
                   >
                   ...
                   {/* Dropdown Menu */}
                   {isFilterMenuOpen && (
                        <div className="absolute right-0 top-full mt-3 w-48 bg-white rounded-2xl shadow-xl border border-gray-100 z-[999] overflow-hidden animate-in fade-in zoom-in-95 p-1 origin-top-right dark:bg-slate-900 dark:border-slate-800">
                            <div className="py-1">
                                <button 
                                    onClick={() => { setSortBy('newest'); setIsFilterMenuOpen(false); }} 
                                    className={`w-full text-left px-4 py-3 rounded-xl text-sm font-bold flex items-center justify-between ${sortBy === 'newest' ? 'bg-emerald-50 text-emerald-700' : 'text-gray-700 hover:bg-gray-50'} dark:bg-slate-800 dark:text-slate-300`}
                                >
                                    ថ្មីបំផុត
                                ...
                                <button 
                                    ...
                                    className={`w-full text-left px-4 py-3 rounded-xl text-sm font-bold flex items-center justify-between ${sortBy === 'popular' ? 'bg-emerald-50 text-emerald-700' : 'text-gray-700 hover:bg-gray-50'} dark:bg-slate-800 dark:text-slate-300`}
                                >
                                    ពេញនិយម
                                ...
                                <button 
                                    ...
                                    className={`w-full text-left px-4 py-3 rounded-xl text-sm font-bold flex items-center justify-between ${sortBy === 'likes' ? 'bg-emerald-50 text-emerald-700' : 'text-gray-700 hover:bg-gray-50'} dark:bg-slate-800 dark:text-slate-300`}
                                >
                                    ចូលចិត្តច្រើន
         ...
         <div className="flex-1 overflow-y-auto p-4 md:p-8 no-scrollbar scroll-smooth">
            <div className="w-full pb-8">
               ...
               <div className="flex items-center justify-between mb-6">
                  <h2 className="text-xl font-bold text-gray-900 font-khmer flex items-center gap-2 dark:text-white">
               ...
               {filterMode !== 'authors' && filteredBooks.length === 0 && (
                   <div className="text-center py-24">
                       ...
                       <p className="font-bold text-gray-500 text-lg dark:text-slate-400">មិនមានសៀវភៅទេ</p>
               ...
               <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
                   {filteredBooks.map((book, idx) => {
                       ...
                       <div 
                          ...
                          className="bg-white rounded-3xl p-3.5 shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 cursor-pointer group flex flex-col h-full border border-gray-100/80 dark:bg-slate-900 dark:border-slate-800"
                          style={{ animationDelay: `${idx * 50}ms` }}
                       >
                          {/* Cover Image */}
                          <div className="aspect-[3/4] rounded-2xl bg-gray-50 overflow-hidden relative mb-4 shadow-sm border border-gray-100/50 dark:bg-slate-800 dark:border-slate-800">
                              ...
                              <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-center pb-4 gap-3">
                                  <button 
                                      ...
                                      className="p-2.5 bg-white/90 backdrop-blur-sm rounded-full text-gray-700 hover:text-emerald-600 hover:scale-110 transition-all shadow-lg dark:bg-slate-900 dark:text-slate-300"
                                      title="Download"
                                  >
                                  ...
                                  <button 
                                      ...
                                      className="p-2.5 bg-white/90 backdrop-blur-sm rounded-full text-gray-700 hover:text-amber-500 hover:scale-110 transition-all shadow-lg dark:bg-slate-900 dark:text-slate-300"
                                      title="Save"
                                  >
                          ...
                          <div className="flex-1 flex flex-col px-1">
                              <h3 className="font-bold text-gray-900 font-khmer line-clamp-2 leading-snug mb-1.5 group-hover:text-emerald-600 transition-colors dark:text-white">
                                  {book.title}
                              </h3>
                              <p className="text-xs text-gray-500 line-clamp-1 mb-4 font-medium dark:text-slate-400">{book.author}</p>

                              <div className="mt-auto pt-3 border-t border-gray-100/80 flex items-center justify-between dark:border-slate-800">
                                  <div 
                                  ...
                                  >
                                      ...
                                      <span className="text-[10px] font-bold text-gray-500 truncate max-w-[80px] group-hover/author:text-emerald-600 transition-colors dark:text-slate-400">{book.uploader.name}</span>
                                  </div>
                                  
                                  <div className="flex items-center gap-1.5 text-[10px] font-bold text-gray-400 bg-gray-50/80 px-2.5 py-1.5 rounded-lg border border-gray-100/50 dark:bg-slate-800 dark:border-slate-800">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                         {distinctAuthors.map(author => (
                            <div 
                               ...
                               className="bg-white p-4 rounded-3xl border border-gray-100/80 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer flex flex-col items-center text-center group relative overflow-hidden dark:bg-slate-900 dark:border-slate-800"
                            >
                               ...
                               <div className="relative mb-3">
                                   <div className="w-16 h-16 rounded-full p-1 bg-white shadow-sm ring-2 ring-transparent group-hover:ring-emerald-100 transition-all duration-300 dark:bg-slate-900">
                               ...
                               <div className="relative z-10 w-full">
                                   <h4 className="font-bold text-gray-900 text-sm truncate w-full group-hover:text-emerald-600 transition-colors dark:text-white">{author.name}</h4>
                                   <p className="text-[10px] text-gray-500 mt-0.5 uppercase tracking-wide font-medium dark:text-slate-400">{author.role}</p>
         ...
         {selectedBook && (
             <BookReader 
               ...
               className={`fixed md:absolute inset-0 z-[1000] md:z-[100] bg-white flex flex-col animate-in slide-in-from-bottom duration-300 overflow-hidden dark:bg-slate-900`}
             />
         )}
         ...
         {selectedAuthor && (
             <AuthorProfileView 
               ...
               className="absolute inset-0 z-[100] bg-gray-50 flex flex-col animate-in slide-in-from-right duration-300 overflow-y-auto dark:bg-slate-800"
             />
         )}
      </div>
        ...
        {isMobileMenuOpen && (
          <div className="fixed inset-0 z-[9999] lg:hidden">
             ...
             <motion.div 
                ...
                className="absolute left-0 md:left-20 top-0 bottom-0 w-72 bg-white shadow-2xl flex flex-col pt-6 dark:bg-slate-900"
             >
